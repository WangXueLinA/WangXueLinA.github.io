...
[一个ts类型相关的刷题项目](https://github.com/type-challenges/type-challenges/blob/main/README.zh-CN.md)
...
在基础类型上，typeScript增添了void、any、enum等原始类型
...
数字类型和javaScript一样，typeScript的数值类型都是浮点数，可支持二进制、八进制、十进制和十六进制
...
跟javaScript一样，可以使用双引号（" "）或单引号（' '）表示字符串，也可以使用模版字符串``进行包裹
...
任何类型也都可以是unknown类型，但与any完全相反，unknown类型就像是typeScript给打上了一个重点检查的标签。在没有对它进行类型检查之前，unknow类型的变量是不能进行任何操作的。
...
那如何使unknown类型能正常使用呢  
让typeScript编译器"看到"并且"相信"你的操作是合法安全的。
...
ts中的viod表示没有任何类型，一般用于定义方法的时候方法没有返回值
